# -*- coding: utf-8 -*-
"""
Created on Tue May 25 16:18:02 2021
...
#epsilon = 3.0 * np.random.rand(6)
r = np.ones(3)
epsilon = 0.3 * np.ones(6)
...
for i in range(0,m):  
    ...
    miuY_plus = hp.f_clutch(miuX_plus)
    miuY_minus = hp.f_clutch(miuX_minus)
    ...
    Ccontrol_plus_L = hp.Ccontrol_as(GL,HL,VL,GR,HR,VR,epsiloni_add_small_number_L,m)
    Ccontrol_minus_L = hp.Ccontrol_as(GL,HL,VL,GR,HR,VR,epsiloni_minus_small_number_L,m)
    ...
    miuY_plus_L = hp.f_clutch(miuX_plus_L)
    miuY_minus_L = hp.f_clutch(miuX_minus_L)
    ...
    epsilonL_i = epsilon[i]
    epsilonR_i = epsilon[i+m]
    dCcontrol_depsiloni_L_v = hp.dCcontrol_depsiloni(HL[i],VL[i],epsilonL_i,symmetry=False)
    ...
    Ccontrol_plus_R = hp.Ccontrol_as(GL,HL,VL,GR,HR,VR,epsiloni_add_small_number_R,m)
    Ccontrol_minus_R = hp.Ccontrol_as(GL,HL,VL,GR,HR,VR,epsiloni_minus_small_number_R,m)
    ...
    miuY_plus_R = hp.f_clutch(miuX_plus_R)
    miuY_minus_R = hp.f_clutch(miuX_minus_R)
    ...
    epsilonL_i = epsilon[i]
    epsilonR_i = epsilon[i+m]
    dCcontrol_depsiloni_R_v = hp.dCcontrol_depsiloni(HR[i],VR[i],epsilonR_i,symmetry=False)
